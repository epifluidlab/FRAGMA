...
def fetch_fragment_information(tbx_fragment, chrom, location):
    fragments=[]
    tbx_fragment = pysam.TabixFile(tbx_fragment)
    for row in tbx_fragment.fetch(chrom, location-5, location+5):
...
def process_line(line, tbx_line, minimum_fragments):
    ...
    if methylation >= 70 or methylation <= 30:
        pos_fragments, total_fragments_positive = fetch_fragment_information(tbx_line, chrom, location_pos)
        neg_fragments, total_fragments_negative = fetch_fragment_information(tbx_line, chrom, location_neg)
        if total_fragments_positive > minimum_fragments and total_fragments_negative > minimum_fragments:
            ...
            for i in range(-5, 6):
                positive_array[i] /= total_fragments_positive
                positive_array[i] *= 100
                negative_array[i] /= total_fragments_negative
                negative_array[i] *= 100
            return f"{chrom}\t{location_pos}\t{methylation}\t{pos_strand}\t{neg_strand}\t{positive_array}\t{negative_array}\n"
